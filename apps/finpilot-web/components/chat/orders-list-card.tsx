...
export function OrdersListCard({ orders }: { orders: ListOrdersItem[] }) {
  ...
  return (
    <div className="surface flex w-full max-w-md flex-col divide-y divide-border/70 overflow-hidden">
      {orders.map((order) => (
        <div key={order.order_id} className="flex items-center justify-between gap-3 px-4 py-3">
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium">{order.product_name}</p>
            <p className="truncate text-xs text-muted-foreground">
              {order.merchant_name} ·{" "}
              <span className="numeric">₹{order.amount_rupees.toLocaleString("en-IN")}</span> ·{" "}
